...
#define min(x, y) (((x)<(y))?(x):(y))
	...
	void output_target(double value, FILE *outf);

	void output(int index, double value, FILE *outf);
	...
	int main(int argc, char **argv) {
		...
		if (outf == NULL) {
			fprintf(stderr, "can't open outf file %s\n", argv[i + 1]);
			exit(1);
		}
